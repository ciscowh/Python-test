...
ipv4_gw_search = re.search('via\s?\d+\.(\d+\.\d+\.(\d+))',ip_route_result).groups()[1]
ipv4_gw = re.match('(\d+\.\d+\.\d+\.)',ipv4_add).group()+ipv4_gw_search
#打印网关IP地址
print('\n''我们假设网关IP为最后一位'+ipv4_gw_search+'因此网关IP地址为:'+ ipv4_gw +'\n')
#ping网关
